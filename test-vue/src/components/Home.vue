...
export default {
  ...
  methods:{
   getData:function(){
   	var that = this;
	    axios.get("/static/Home.json").then(function(response){
			for(var i = 0; i < response.data.todos.length;i++){
//				console.log(response.data.todos[i])
				that.todos.push(response.data.todos[i])
			}
    ...
    xuangou:function(){
      ...
      console.log(this.$router.push({path:"choose"}))
    },
    parts:function(){
