...
export default {
    ...
    methods:{
        getData:function(){
            var _this=this;
            axios.get("/static/parts.json").then(function(res){
            	console.log(res)
              for(var i=0,len=res.data.list.length;i<len;i++){
                  var selData=res.data.list[i];
                  var part=res.data.list[i].name;
                  _this.list.push(selData)
              }
              for(var i=0,len=res.data.charger.length;i<len;i++){
                  var selData=res.data.charger[i];
                  var part=res.data.charger[i].name;
                  _this.charger.push(selData)
              }
              for(var i=0,len=res.data.protect.length;i<len;i++){
                  var selData=res.data.protect[i];
                  var part=res.data.protect[i].name;
                  _this.protect.push(selData)
              }
              for(var i=0,len=res.data.headset.length;i<len;i++){
                  var selData=res.data.headset[i];
                  var part=res.data.headset[i].name;
