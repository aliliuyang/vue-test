...
export default {
  ...
  methods:{
      getData:function(){
          var _this=this;
          axios.get("/static/choose.json").then(function(res){
              for(var i= 0; i< res.data.MotoZs.length;i++){
              	_this.MotoZs.push(res.data.MotoZs[i])
              }
              for(var i= 0; i< res.data.motos.length;i++){
              	_this.motos.push(res.data.motos[i])
              }
              for(var i= 0; i< res.data.chooses.length;i++){
              	_this.chooses.push(res.data.chooses[i])
              }
              for(var i= 0; i< res.data.z2s.length;i++){
              	_this.z2s.push(res.data.z2s[i])
              }
